...
export default class StrengthUI {
    constructor(scene, x, y, initialStrength, config = {}) {
        ...
        ALLOWED_STRENGTHS.forEach((strength, index) => {
            ...
            const totalWidth = ALLOWED_STRENGTHS.length * buttonWidth +
    ...
    updateStrengthIndicator(strength) {
        if (!ALLOWED_STRENGTHS.includes(strength)) {
            console.warn(`Invalid strength value: ${strength}. Expected one of ${ALLOWED_STRENGTHS.join(", ")}. Defaulting to the lowest...`);
